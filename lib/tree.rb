module Tree
  ...
  def media_count(options={})
    if options[:type].nil?
      ...
      count = Medium.count(:all, :conditions => {'cumulative_media_category_associations.category_id' => self.id, 'media.type' => options[:type]}, :joins => :cumulative_media_category_associations, :select => 'media.id')
    end
    count
  end
