class AddFeatureIdToMediaAdministrativeLocation < ActiveRecord::Migration
  ...
  def self.up
    change_table :media_administrative_locations do |t|
      ...
      MediaAdministrativeLocation.order(:id).each { |location| location.update_attribute(:feature_id, location.administrative_unit.feature_id) }
  ...
  def self.down
    ...
    change_table :media_administrative_locations do |t|
      ...
      MediaAdministrativeLocation.order(:id).each { |location| location.update_attribute(:administrative_unit_id, AdministrativeUnit.find_by(feature_id: location.feature_id)).id }
