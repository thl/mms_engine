class CreateCumulativeMediaCategoryAssociations < ActiveRecord::Migration
  def self.up
    ...
    MediaCategoryAssociation.find(:all, :group =>'category_id', :order => 'category_id').collect(&:category).each do |category|
      medium_ids = MediaCategoryAssociation.find(:all, :conditions => {:category_id => category.id}).collect(&:medium_id)
